...
function createPurchaseOrder(payload) {
  ...
  const taxRate = number_(input.tax_rate, 0);
...
function createSalesOrder(payload) {
  ...
  const tax = input.tax_enabled ? subtotal * number_(input.tax_rate, 0) / 100 : number_(input.tax_amount, 0);
  const shipping = number_(input.shipping_amount, 0);
  const order = {
    ...
    tax_rate: number_(input.tax_rate, 0),
...
function inventorySnapshot() {
  ...
  readTable_("INVENTORY_MOVEMENTS").forEach((move) => {
    ...
    const productId = move.product_id || (lots[lotId] || {}).product_id || "";
    const locationId = move.to_location_id || (lots[lotId] || {}).current_location_id || "";
    ...
    grouped[key].current_qty += number_(move.qty_change, 0);
  });
...
function getDashboard() {
  ...
  return {
    ...
    inventoryValue: snapshot.reduce((sum, row) => sum + number_(row.current_qty, 0) * number_((lotMap[row.internal_lot_id] || {}).unit_cost, 0), 0),
